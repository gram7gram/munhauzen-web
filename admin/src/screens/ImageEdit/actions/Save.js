...
export default model => (dispatch, getState) => {
  ...
  let promise
  if (model._id) {
    promise = axios(getState()).put(`${parameters.apiHost}/api/v1/${locale}/images/${model._id}`, data)
  } else {
    promise = axios(getState()).post(`${parameters.apiHost}/api/v1/${locale}/images`, data)
  }
